...
describe "ReviewComparator" do
	context "calculate_similarity" do
		...
		it "should work set negative similarity ratings to 0" do
			...
			similarity_score.should == 0
		end

		it "should raise an exception when the review counts are not equal" do
			review1 = Review.create :movie => Movie.create(:title => "Test"), :rating => 2, :confidence => 3, :user => @tom
			review2 = Review.create :movie => Movie.create(:title => "Test2"), :rating => 5, :confidence => 2, :user => @tom
